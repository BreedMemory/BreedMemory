...
public class InjectUtils {
	...
	private static Logger logger = new Logger("InjectUtils");

	/**
	 * 注入（布局、控件、事件）
	 * @param activity 需要注入的类
	 */
	public static void inject(CkActivity activity){
	...
	public static void injectContentView(CkFragment fragment) {
		...
		for (Class<?> clazz : classes ) {
			try {
				...
				View v = inflater.inflate(layoutId, null);
				if(fragment.getmRootView() != null && ViewGroup.class.isAssignableFrom(fragment.getmRootView().getClass())) {
					((ViewGroup)fragment.getmRootView()).addView(v, 0);
				} else {
					fragment.setmRootView(v);
				}
			} catch (Exception e) {
				logger.i("inject content view failed!", e);
				return;
