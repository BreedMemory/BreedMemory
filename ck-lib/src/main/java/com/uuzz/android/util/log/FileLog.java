...
public class FileLog {
    ...
    /** 是否有权限输出日志,默认为false */
    private static boolean HAVE_PERMISSION = false;
    /** 日志文件大小最大值,单位为M */
    ...
    public static FileLog getInstance() {
        if(!IS_OUTPUT || HAVE_PERMISSION){    //如果不让输出则直接返回空对象
            return mInstance;
        }
        if(mInstance == null) {
     ...
     * 描 述：设置io权限<br/>
     * 作 者：谌珂<br/>
     * 历 史: (1.0.0) 谌珂 2016/8/29 <br/>
     */
    public void setIoPermission(boolean permission) {
    ...
    private synchronized void flushLog(){
        if(!IS_OUTPUT || HAVE_PERMISSION){    //如果不让输出则直接返回
            return;
        }
        isDirectWrite = true;
